pub fn part_a(input: &str) -> i32 {
    ...
    for line in input.lines() {
        ...
        for round in rounds {
            ...
            for cube in round.split(',') {
                ...
                let cube_count: i32 = cube_parts[0].parse().expect("should be a number");
                match cube_color {
                    "red" => red += cube_count,
    ...
    for game in &game_history {
        if game.rounds.iter().all(|round| {
            round.red <= red_cube_max
                && round.green <= green_cube_max
                && round.blue <= blue_cube_max
        }) {
...
pub fn part_b(input: &str) -> i32 {
    ...
    for line in input.lines() {
        ...
        game_history.push(Game {
            min_red: min_red.abs(),
            min_green: min_green.abs(),
            min_blue: min_blue.abs(),
        });
    }
